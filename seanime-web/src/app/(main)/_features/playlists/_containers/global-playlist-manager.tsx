import { Anime_Entry, Anime_Playlist, Anime_PlaylistEpisode, HibikeTorrent_AnimeTorrent, HibikeTorrent_BatchEpisodeFiles } from "@/api/generated/types"
...
export function GlobalPlaylistManager() {
    ...
    function sameStream(autoPlayStream: { entry: Anime_Entry, torrent: HibikeTorrent_AnimeTorrent } | null, episode: Anime_PlaylistEpisode) {
        if (!autoPlayStream) return false

        return autoPlayStream.entry.mediaId == episode.episode?.baseAnime?.id
    }
                    ...
                    switch (payload2.playlistEpisode.watchType) {
                        ...
                        case "debrid":
                            if (debridStream_currentSessionAutoSelect) {
                                log.info("Auto select is enabled, auto-selecting stream for debrid stream")
                                ...
                                if (autoPlayStream?.torrent?.isBatch && debridStream_autoSelectFile && sameStream(autoPlayStream, episode)) {
                                    ...
                                    // Debrid search is handled by debrid-stream-page
                                    router.push("/entry?id=" + episode.episode?.baseAnime?.id + "&tab=debridstream&episode=" + episode.episode?.episodeNumber)
                                    return
                                }
                            }
                            break
            ...
            <div className="max-w-full space-y-1">
                ...
                <div>
                    <div className="text-xs text-[--muted] line-clamp-1 tracking-wide">
                        {episode.watchType === "debrid" ? "Debrid streaming" :
