...
export function useHandleStartDebridStream() {
    ...
    const { streamStreamingPlayback, electronPlaybackMethod } = useCurrentDevicePlaybackSettings()
    ...
    const getPlaybackType = React.useCallback((forcePlaybackMethod?: ForcePlaybackMethod) => {
        ...
        if (!!externalPlayerLink?.length && (
            (!forcePlaybackMethod && streamStreamingPlayback === PlaybackStreamStreaming.ExternalPlayerLink) ||
            (forcePlaybackMethod && forcePlaybackMethod === "externalPlayerLink")
        )) {
            return "externalPlayerLink"
        }
        return "default"
    }, [externalPlayerLink, streamStreamingPlayback, electronPlaybackMethod])
