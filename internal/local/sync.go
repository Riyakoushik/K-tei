...
// synchronizeCollections should be called after the tracked anime snapshots have been updated.
// The ManagerImpl.animeCollection should be set & up-to-date.
...
func (q *Syncer) refreshCollections() {
	...
	if len(q.animeJobQueue) > 0 {
		q.manager.logger.Trace().Msg("local manager: Skipping refreshCollections, job queue is not empty")
		return
	}
...
func (q *Syncer) runDiffs(
	...
	if len(q.animeJobQueue) > 0 {
		q.manager.logger.Trace().Msg("local manager: Skipping diffs, job queue is not empty")
		return
	}
